...
public class NodeService : INodeService
{
    ...
    public async Task<NodeDto> RegisterOrUpdateAsync(CreateNodeDto dto, CancellationToken ct = default)
    {
        ...
        if (existingNode != null)
        {
            // Update existing node - keep existing name unless explicitly provided
            existingNode.LastSeen = DateTime.UtcNow;
            existingNode.IsOnline = true;

            // Only update name if explicitly provided (preserves user-configured names)
        ...
        // Create new Node - generate name if not provided
        var createDtoWithName = dto with
        {
            Name = dto.Name ?? GenerateNameFromNodeId(dto.NodeId)
        };
        return await CreateAsync(createDtoWithName, ct);
    }
    ...
    public async Task<(NodeDto Node, bool IsNew)> RegisterOrUpdateWithStatusAsync(CreateNodeDto dto, string? firmwareVersion = null, CancellationToken ct = default)
    {
        ...
        if (existingNode != null)
        {
            // Update existing node - keep existing name unless ESP32 explicitly provides one
            existingNode.LastSeen = DateTime.UtcNow;
            existingNode.IsOnline = true;

            // Only update name if ESP32 explicitly sent a name (not auto-generated by controller)
            // This preserves user-configured names in the Hub UI
            if (!string.IsNullOrEmpty(dto.Name))
            {
                _logger.LogDebug("Node {NodeId}: Updating name from '{OldName}' to '{NewName}'",
                    dto.NodeId, existingNode.Name, dto.Name);
                existingNode.Name = dto.Name;
            }
            else
            {
                _logger.LogDebug("Node {NodeId}: Keeping existing name '{Name}' (no name provided by ESP32)",
                    dto.NodeId, existingNode.Name);
            }
        ...
        var newNode = await CreateAsync(createDtoWithName, ct);
        return (newNode, true);
    }
